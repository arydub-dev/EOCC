...
class Settings(BaseSettings):
    ...
    def fernet_key(self) -> bytes:
        ...
        - If ENCRYPTION_KEY is already a valid Fernet key (urlsafe-b64 of exactly
          32 bytes), use it verbatim.
        - If ENCRYPTION_KEY is set but is an arbitrary secret (e.g. a value
          generated by a platform like Render), derive a valid key from it.
        - If ENCRYPTION_KEY is blank, derive one from SECRET_KEY (dev only;
          production requires ENCRYPTION_KEY, enforced above).
        """
        if self.ENCRYPTION_KEY:
            try:
                if len(base64.urlsafe_b64decode(self.ENCRYPTION_KEY)) == 32:
                    return self.ENCRYPTION_KEY.encode()
            except (ValueError, TypeError):
                pass
            seed = self.ENCRYPTION_KEY
        else:
            seed = self.SECRET_KEY + ":fernet"
