...
class Organization(Base):
    ...
    plan: Mapped[enums.Plan] = mapped_column(_enum(enums.Plan), default=enums.Plan.STARTER, nullable=False)
...
class User(Base):
    ...
    org: Mapped[Optional["Organization"]] = relationship(back_populates="users")
...
class UserSession(Base):
    ...
    replaced_by_id: Mapped[Optional[int]] = mapped_column(Integer)

    ip_address: Mapped[Optional[str]] = mapped_column(String(64))
    user_agent: Mapped[Optional[str]] = mapped_column(String(400))
    device_label: Mapped[Optional[str]] = mapped_column(String(160))
...
class Incident(Base):
    ...
    data_classification: Mapped[enums.DataClassification] = mapped_column(
        _enum(enums.DataClassification), default=enums.DataClassification.CONFIDENTIAL, nullable=False
    )
    ...
    incident_type: Mapped[enums.IncidentType] = mapped_column(_enum(enums.IncidentType), nullable=False, index=True)
    ...
    severity_score: Mapped[float] = mapped_column(Float, default=0.0, nullable=False)  # computed 0-100
...
class Resource(Base):
    ...
    name: Mapped[str] = mapped_column(String(255), nullable=False, index=True)
    resource_type: Mapped[enums.ResourceType] = mapped_column(_enum(enums.ResourceType), nullable=False, index=True)
    status: Mapped[enums.ResourceStatus] = mapped_column(
        _enum(enums.ResourceStatus), default=enums.ResourceStatus.AVAILABLE, nullable=False, index=True
    )
    region: Mapped[Optional[str]] = mapped_column(String(120), index=True)
    home_base: Mapped[Optional[str]] = mapped_column(String(255))

    latitude: Mapped[float] = mapped_column(Float, nullable=False)
    longitude: Mapped[float] = mapped_column(Float, nullable=False)

    capacity: Mapped[float] = mapped_column(Float, default=0.0, nullable=False)
    capacity_unit: Mapped[Optional[str]] = mapped_column(String(40))
...
class UtilityOutage(Base):
    ...
    utility_type: Mapped[enums.UtilityType] = mapped_column(_enum(enums.UtilityType), nullable=False, index=True)
    status: Mapped[enums.UtilityOutageStatus] = mapped_column(
        _enum(enums.UtilityOutageStatus), default=enums.UtilityOutageStatus.REPORTED, nullable=False, index=True
    )
    region: Mapped[Optional[str]] = mapped_column(String(120), index=True)
    latitude: Mapped[float] = mapped_column(Float, nullable=False)
    longitude: Mapped[float] = mapped_column(Float, nullable=False)
    ...
    description: Mapped[Optional[str]] = mapped_column(Text)
    started_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    estimated_restoration: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True))
    incident_id: Mapped[Optional[int]] = mapped_column(ForeignKey("incidents.id", ondelete="SET NULL"))
...
class RiskAssessment(Base):
    ...
    category: Mapped[enums.RiskCategory] = mapped_column(_enum(enums.RiskCategory), nullable=False, index=True)
    severity: Mapped[enums.RiskSeverity] = mapped_column(_enum(enums.RiskSeverity), nullable=False, index=True)
    ...
    incident: Mapped[Optional["Incident"]] = relationship(back_populates="risk_assessments")
...
class Alert(Base):
    ...
    category: Mapped[enums.AlertCategory] = mapped_column(_enum(enums.AlertCategory), nullable=False, index=True)
    severity: Mapped[enums.AlertSeverity] = mapped_column(_enum(enums.AlertSeverity), nullable=False, index=True)
    ...
    incident_id: Mapped[Optional[int]] = mapped_column(ForeignKey("incidents.id", ondelete="SET NULL"))
    hospital_id: Mapped[Optional[int]] = mapped_column(ForeignKey("hospitals.id", ondelete="SET NULL"))
    shelter_id: Mapped[Optional[int]] = mapped_column(ForeignKey("shelters.id", ondelete="SET NULL"))

    acknowledged_by_id: Mapped[Optional[int]] = mapped_column(ForeignKey("users.id", ondelete="SET NULL"))
    acknowledged_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True))
    resolved_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True))
    response_actions: Mapped[Optional[list]] = mapped_column(JSON)
...
class Simulation(Base):
    ...
    name: Mapped[str] = mapped_column(String(255), nullable=False)
    simulation_type: Mapped[enums.SimulationType] = mapped_column(_enum(enums.SimulationType), nullable=False, index=True)
    ...
    created_by: Mapped[Optional["User"]] = relationship(back_populates="simulations")
...
class AIReport(Base):
    ...
    grounding: Mapped[Optional[dict]] = mapped_column(JSON)
...
class DataSource(Base):
    ...
    name: Mapped[str] = mapped_column(String(255), nullable=False)
    source_type: Mapped[enums.DataSourceType] = mapped_column(_enum(enums.DataSourceType), nullable=False, index=True)
    ...
    auth_type: Mapped[Optional[str]] = mapped_column(String(40))
...
class ImportJob(Base):
    ...
    data_source_id: Mapped[Optional[int]] = mapped_column(ForeignKey("data_sources.id", ondelete="SET NULL"))
    source_type: Mapped[enums.DataSourceType] = mapped_column(_enum(enums.DataSourceType), nullable=False)
    target_entity: Mapped[str] = mapped_column(String(80), nullable=False)
    status: Mapped[enums.ImportJobStatus] = mapped_column(
        _enum(enums.ImportJobStatus), default=enums.ImportJobStatus.PENDING, nullable=False, index=True
    )
    ...
    data_source: Mapped[Optional["DataSource"]] = relationship(back_populates="import_jobs")
