...
def list_resources(
    ...
    return Page.create([ResourceOut.model_validate(r) for r in items], total, params.page, params.page_size)


@router.get("/utilization")
def utilization_analytics(db: Session = Depends(get_db), _: User = Depends(get_current_user)) -> dict:
    rows = db.execute(
    ...
    for bucket in by_type.values():
        ...
        bucket["availability_pct"] = round((available / bucket["total"] * 100) if bucket["total"] else 0, 1)
        assigned = sum(
            bucket["by_status"].get(s, 0) for s in ("assigned", "en_route", "on_scene")
        )
        bucket["utilization_pct"] = round((assigned / bucket["total"] * 100) if bucket["total"] else 0, 1)
    return {"by_type": by_type}


@router.get("/{resource_id}", response_model=ResourceOut)
def get_resource(resource_id: int, db: Session = Depends(get_db), _: User = Depends(get_current_user)) -> Resource:
...
def create_resource(
    ...
    db.commit()
    db.refresh(resource)
    audit_service.log(db, actor=user, action="create_resource", entity_type="resource", entity_id=resource.id)
    return resource


@router.patch("/{resource_id}", response_model=ResourceOut)
def update_resource(
    resource_id: int, payload: ResourceUpdate, db: Session = Depends(get_db), user: User = Depends(require_manager)
) -> Resource:
    resource = db.get(Resource, resource_id)
    if not resource:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Resource not found")
    ...
    db.commit()
    db.refresh(resource)
    audit_service.log(db, actor=user, action="update_resource", entity_type="resource", entity_id=resource.id)
    return resource


@router.post("/assignments", response_model=ResourceAssignmentOut, status_code=status.HTTP_201_CREATED)
def assign_resource(
    payload: ResourceAssignmentCreate, db: Session = Depends(get_db), user: User = Depends(require_manager)
) -> ResourceAssignment:
    ...
    audit_service.log(
        db, actor=user, action="assign_resource", entity_type="resource_assignment", entity_id=assignment.id,
...
def release_assignment(
    ...
    db.refresh(assignment)
    audit_service.log(
        db, actor=user, action="release_assignment", entity_type="resource_assignment", entity_id=assignment.id
    )
    return assignment
